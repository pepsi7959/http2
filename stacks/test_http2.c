...
,0x73,0x3d,0x2a,0x29,0x32,0x01,0x2a}; 
    ...
    // READ //
    DEBUG("Reading");
    if( HTTP2_read(hc, conn, error)  != HTTP2_RET_OK ) printf("read : %s\n",error);
    if ( HTTP2_decode(hc, conn, error) != HTTP2_RET_OK ) printf("decode : %s\n",error);
...
int test_HTTP2_write_header(){
    ...
    ASSERT( HTTP2_write_header(hc, conn, &hb, &hf1, error) == HTTP2_RET_OK);
    ASSERT( memcmp(hb->data, header_frame, hb->len) == 0);
    
    HEADER_FIELD hf2 = {NULL,NULL,0,0,":scheme", "http"};
    ASSERT( HTTP2_write_header(hc, conn, &hb, &hf2, error) == HTTP2_RET_OK);
    ASSERT( memcmp(hb->data, header_frame, hb->len) == 0);
    
    HEADER_FIELD hf3 = {NULL,NULL,0,0,":path", "/pb.D21/Do"};
    ASSERT( HTTP2_write_header(hc, conn, &hb, &hf3, error) == HTTP2_RET_OK);
    ASSERT( memcmp(hb->data, header_frame, hb->len) == 0);
    
    HEADER_FIELD hf4 = {NULL,NULL,0,0,":authority", "10.252.169.10"};
    ASSERT( HTTP2_write_header(hc, conn, &hb, &hf4, error) == HTTP2_RET_OK);
    ASSERT( memcmp(hb->data, header_frame, hb->len) == 0);
    
    HEADER_FIELD hf5 = {NULL,NULL,0,0,"content-type", "application/grpc"};
    ASSERT( HTTP2_write_header(hc, conn, &hb, &hf5, error) == HTTP2_RET_OK);
    ASSERT( memcmp(hb->data, header_frame, hb->len) == 0);
    
    HEADER_FIELD hf6 = {NULL,NULL,0,0,"user-agent", "grpc-go/0.11"};
    ASSERT( HTTP2_write_header(hc, conn, &hb, &hf6, error) == HTTP2_RET_OK);
    ASSERT( memcmp(hb->data, header_frame, hb->len) == 0);
    
    HEADER_FIELD hf7 = {NULL,NULL,0,0,"te", "trailers"};
    ASSERT( HTTP2_write_header(hc, conn, &hb, &hf7, error) == HTTP2_RET_OK);
    HEXDUMP(hb->data, hb->len);
    ASSERT( memcmp(hb->data, header_frame, hb->len) == 0);

    HEADER_FIELD hf71 = {NULL,NULL,0,0,"grpc-timeout", "499924u"};
    ASSERT( HTTP2_write_header(hc, conn, &hb, &hf71, error) == HTTP2_RET_OK);
    HEXDUMP(hb->data, hb->len);
    ASSERT( memcmp(hb->data, header_frame, hb->len) == 0);
    
    //header 2
     HEADER_FIELD hf8 = {NULL,NULL,0,0,":method", "POST"};
    ASSERT( HTTP2_write_header(hc, conn, &hb, &hf8, error) == HTTP2_RET_OK);
    HEXDUMP(hb->data, hb->len);
    ASSERT( memcmp(hb->data, header_frame, hb->len) == 0);
    
    HEADER_FIELD hf9 = {NULL,NULL,0,0,":scheme", "http"};
    ASSERT( HTTP2_write_header(hc, conn, &hb, &hf9, error) == HTTP2_RET_OK);
    HEXDUMP(hb->data, hb->len);
    ASSERT( memcmp(hb->data, header_frame, hb->len) == 0);
    
    HEADER_FIELD hf10 = {NULL,NULL,0,0,":path", "/pb.D21/Do"};
    ASSERT( HTTP2_write_header(hc, conn, &hb, &hf10, error) == HTTP2_RET_OK);
    HEXDUMP(hb->data, hb->len);
    ASSERT( memcmp(hb->data, header_frame, hb->len) == 0);
    
    HEADER_FIELD hf11 = {NULL,NULL,0,0,":authority", "10.252.169.10"};
    ASSERT( HTTP2_write_header(hc, conn, &hb, &hf11, error) == HTTP2_RET_OK);
    HEXDUMP(hb->data, hb->len);
    ASSERT( memcmp(hb->data, header_frame, hb->len) == 0);
    
    HEADER_FIELD hf12 = {NULL,NULL,0,0,"content-type", "application/grpc"};
    ASSERT( HTTP2_write_header(hc, conn, &hb, &hf12, error) == HTTP2_RET_OK);
    HEXDUMP(hb->data, hb->len);
    ASSERT( memcmp(hb->data, header_frame, hb->len) == 0);
    
    HEADER_FIELD hf13 = {NULL,NULL,0,0,"user-agent", "grpc-go/0.11"};
    ASSERT( HTTP2_write_header(hc, conn, &hb, &hf13, error) == HTTP2_RET_OK);
    ASSERT( memcmp(hb->data, header_frame, hb->len) == 0);
    
    HEADER_FIELD hf14 = {NULL,NULL,0,0,"te", "trailers"};
    ASSERT( HTTP2_write_header(hc, conn, &hb, &hf14, error) == HTTP2_RET_OK);
    HEXDUMP(hb->data, hb->len);
    ASSERT( memcmp(hb->data, header_frame, hb->len) == 0);
...
/*         unsigned char frame[] = {   0x00,0x00,0x4c,0x01,0x04,0x00,0x00,0x00,
    ...
    ASSERT( GRPC_gen_search_request(&data,"subdata=profile,ds=slf,subdata=services,uid=000000000000001,ds=SUBSCRIBER,o=AIS,DC=C-NTDB", "search", "(objectClass=*)", NULL, 0, error) == GRPC_RET_OK);
    ...
    //DELETE
    //ASSERT( GRPC_gen_delete_request(&data,"systemId=ocf,subdata=profile,ds=slf,subdata=services,uid=000000000000001,ds=SUBSCRIBER,o=AIS,DC=C-NTDB", 0, error) == GRPC_RET_OK);
    
    //ADD
    ASSERT( GRPC_gen_add_request(&data,"systemId=ocf,subdata=profile,ds=slf,subdata=services,uid=000000000000001,ds=SUBSCRIBER,o=AIS,DC=C-NTDB", NULL, 0, error) == GRPC_RET_OK);
    HTTP2_send_message(hc, conn, hb, data, error);
    sleep(1);
    ASSERT( HTTP2_write(hc, conn , error) == HTTP2_RET_SENT );
    ...
    /*     hb->len = 0;
        ...
        ASSERT( GRPC_gen_delete_request(&data,"uid=000000000000001,ds=SUBSCRIBER,o=AIS,dc=C-NTDB", 0, error) == GRPC_RET_OK);
        HTTP2_send_message(hc, conn, hb, data, error); */
        HTTP2_write(hc, conn , error);
        sleep(1);
    }
